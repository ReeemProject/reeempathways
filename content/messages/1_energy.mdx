...
3.	A step is to improve representation of some key _regional and local specificities of the energy system development_ of selected EU countries. This part builds on a harmonisation process between TIMES PanEU and case study models focusing on energy supply security and grid and dispatch with the help of the experts in the case study regions, existing base year capacities and electricity trade flow between the regions are improved and the planned capacities from ENTSO-E [19] are incorporated into the model as well. 
